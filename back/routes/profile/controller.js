...
export default {
  protectedUpdate: (body, locals, messageArray) => {
    ...
    if (body.password) {
      if (!(body.oldPswd && locals.user.comparePasswords(body.oldPswd))) {
  ...
  // protectedUpdate: (body, locals) => {
  //   if (body.password) {

  //     bcrypt.compare(body.password, locals.user.password).then(function(ressss) {
  //       console.log('true ? ', ressss)
  //       console.log('compare', body.password, 'encore: ', locals.user.password)
  //     });
  //   }  
  //      else {
  //       locals.user.password = bcrypt.hashSync(body.password, 10)
