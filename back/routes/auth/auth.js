    ...
    User.findOne({ email: rer.body.email }, function (err, user) {
      ...
      if (!user) {
        ...
        if (!user.comparePasswords(req.body.password)) {
          ...
          jwt.sign({ email: user.email, _id: user._id, role: user.role }, process.env.SECRETKEY, function (err, token) {
            if (err) res.status(500).json({success: false, message: err.message})
            else res.status(200).json({success: true, message: 'Enjoy your unlimited access!', content: {token: process.env.AUTHBEARER + ' ' + token, userId: user._id}})
          })
        }
      }
    })
