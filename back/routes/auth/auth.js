...
router.post('/login', (req, res) => {
  if (req.body.email && req.body.password) {
    ...
    res.status(412).json({ success: false, message: 'Имя пользователя и / или пароль отсутствуют. email and/or password are missing.'})
  }
})
  ...
  if (req.body.email && req.body.password) {
    if (helper.regexEmail.test(req.body.email)) {
      User.findOne({ email: helper.caseInsensitive(req.body.email)}, function (err, result) {
        ...
        } else res.status(412).json({ success: false, message: 'Имя пользователя уже используется. email already used.'})
      })
    } else res.status(412).json({ success: false, message: 'Требуется электронная почта. Email required.' })
  } else res.status(412).json({ success: false, message: 'Имя пользователя и / или пароль отсутствуют. email and/or password are missing.'})
})
